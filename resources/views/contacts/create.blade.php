...
<form action="{{ isset($contact) ? route('contacts.update', $contact) : route('contacts.store') }}" method="POST">
    ...
    <input type="text" name="name" value="{{ old('name', $contact->name ?? '') }}" class="w-full p-2 border rounded" required>

    <label class="block mt-4 mb-2">Phone</label>
    <input type="number" name="number" value="{{ old('number', $contact->number ?? '') }}" class="w-full p-2 border rounded" required>
